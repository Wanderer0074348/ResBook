...
interface DotfilesTableProps {
  dotfiles: DotfileFrontmatter[];
}

const columnHelper = createColumnHelper<DotfileFrontmatter>();

export function DotfilesTable({ dotfiles }: DotfilesTableProps) {
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor("title", {
        header: "Dotfile",
        cell: (info) => {
          const dotfile = info.row.original;
          return (
            <Link
  ...
  const kindOptions = Array.from(new Set(dotfiles.map((d) => d.kind)));
    ...
    <div className="space-y-0">
      ...
      <div className="border border-gray-300 p-3 dark:border-gray-700 border-b-0">
        <div className="grid gap-2 grid-cols-1 md:grid-cols-4">
          ...
          <select
            value={
              (table.getColumn("kind")?.getFilterValue() as string) || ""
            }
            onChange={(e) =>
              table.getColumn("kind")?.setFilterValue(e.target.value || undefined)
            }
            className="border border-gray-300 bg-white px-2 py-1 text-xs dark:border-gray-700 dark:bg-black"
          >
